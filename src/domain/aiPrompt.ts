...
- COPYの方向は必ず FROM → TO。FROMは「残したい牌姿が実在する元巡目」、TOは「その牌姿を成立させたい比較先巡目」。終盤14巡目のSELF手牌を対面リーチ直後の8巡目へ移すなら COPY SELF HAND FROM 14 TO 8 であり、FROM 8 TO 14 は逆なので禁止する。
- 相手のリーチ巡目はSELF HANDのコピー元ではない。相手のリーチ時刻はTOを決める材料にし、FROMにはSELFの残したい手牌が実在する後半の巡目を指定する。終盤の手牌を序盤・中盤へ移す比較では通常 FROM の数値 > TO の数値になるため、逆なら出力前に再確認する。
