...
- COPYのデータ移動方向は必ず FROM → TO。FROMは「残したい牌姿が実在する元巡目」、TOは「その牌姿を成立させたい比較先巡目」。FROMとTOの数値の大小は制約ではなく、FROM 8 TO 14もFROM 14 TO 8も用途に応じて正しい。
- 相手のリーチ巡目はSELF HANDのコピー元ではない。相手のリーチ時刻はTOを決める材料にし、FROMにはSELFの残したい手牌が実在する巡目を指定する。今回のように終盤14巡目のSELF手牌を対面リーチ直後の8巡目へ移す意図なら COPY SELF HAND FROM 14 TO 8。逆に早い8巡目の牌姿を14巡目へ移す意図なら FROM 8 TO 14。
- SELFのリーチ・聴牌時期を早める／遅らせる比較は、ユーザーが「同じ手牌」と明記していなくても、元のリーチ打牌後と同一の手牌を別巡目へ移す依頼として扱う。必ず COPY SELF HAND FROM <残したい手牌の元巡目> TO <比較先巡目> を使い、その直後に REACH SELF ON AT <比較先巡目> を置く。
...
- 現在河の [リーチ] が付いたSELF打牌位置と、それ以降にツモ切りで維持された同一手牌はFROM候補。「対面リーチ直後くらい」など相対的な時点はTO候補であり、下記の打牌判断時点から、その相手のリーチ直後にあるSELFの最初の打牌巡目を選ぶ。
...
# COPY方向確認用のリーチ巡目（現在の席対応）
${reachSources}
- 上記は各席の宣言位置。相手の宣言位置をSELF HANDのFROMに使わない。FROM=残したいSELF牌姿の元巡目、TO=比較先のSELF巡目、REACH AT=TOと覚える。
- 例: SELFの14巡目の手牌を8巡目へ移してリーチする場合は「COPY SELF HAND FROM 14 TO 8」の直後に「REACH SELF ON AT 8」。
