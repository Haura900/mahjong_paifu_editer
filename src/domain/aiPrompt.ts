...
- 原則として各SCENEの最初の命令を LOCK SELF HAND ALL にする。SCENE外の直接編集でも、変更命令より前に置く。
- LOCK SELF HAND ALLを、実行不能なSCENEを成立させるために外してはならない。SET、SWAP、自動交換、自動聴牌補正、副露補正のどれも固定SELF手牌へ触れさせない。
- 差替え用の牌が必要なら、下記の「余っている字牌」を極力優先する。
- 元の牌を別巡へ移すときは、元位置を余り字牌へSETし、移動先を元の牌へSETする形を優先する。
- 指定巡目のSELF手牌そのものを別巡目へ移す依頼には COPY SELF HAND FROM <コピー元巡目> TO <コピー先巡目> を使う。COPYは再現後の手牌を内部で固定するため、その前に LOCK SELF HAND ALL を置かない。
...
- DSL上に存在しないSELF RIVER位置を捏造しない。同一SELF手牌を別巡目へ移す場合は COPY SELF HAND FROM ... TO ... を使い、通常ツモのない巡目やツモ切り巡目などCOPYを適用できない場合は、その軸のSCENEを生成しない。
